...
set ILMergeSolution=%1External\ILMerge\ILMerge.exe
...
rem #    run merge cmd
@echo Merging: '"%ILMerge%" /wildcards /targetplatform:%net% /out:"%result%" "%target_dir%*.dll" /lib:%1External\EntityFramework.4.1.10715.0\lib\'
@echo '' 
@echo ''
"%ILMerge%" /wildcards /targetplatform:%net% /out:"%result%" "%target_dir%*.dll" /lib:%1External\EntityFramework.4.1.10715.0\lib\
