    ...
    public static class Logger
    {
        ...
        private static void WriteMessageInThread(object threadInfoObject)
        {
            ...
            if (IsSendToNetwork)
            {
                var queryCommand = new QueryCommand
                {
                    CommandText = sqlLogRecord.CommandText,
                    Created = sqlLogRecord.Created,
                    MethodName = sqlLogRecord.MethodName,
                    MethodBody = sqlLogRecord.MethodBody,
                    ClassName = sqlLogRecord.ClassName,
                    QueryMiliseconds = 0,
                    ResultRowsCount = 0,
                    StackTrace = sqlLogRecord.StackTrace
                };
                CommandSender.SendQueryCommand(queryCommand);
            }
            Nlogger.AddLogMessage(sqlLogRecord);
        ...
        private static string GetMethodBody(MethodBase methodBase)
        {
            return IsEnableDecompiling
                ? Decompiler.GetSourceCode(methodBase.Module.FullyQualifiedName, methodBase.DeclaringType.Name, methodBase)
